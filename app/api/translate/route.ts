...
} from "@/lib/prompts/translateListing";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const system = buildTranslateSystemPrompt(input.platform, input.targetLocales);
    const user = buildTranslateUserPrompt({
      platform: input.platform,
      locales: input.targetLocales,
      source: input.sourceEn,
    });
    ...
    const bulletCount = input.sourceEn.bullets.length;
    const translations = parseAndValidateTranslation(parsed, input.targetLocales, bulletCount);
    return NextResponse.json({ ok: true, traceId, translations });
  } catch (e) {
